...
- **每池可配置延迟阈值**：自动择优模式下，超过阈值（ms）的模型排到最后（0=不限，默认 3000ms），嵌套子池用各自阈值
- **兜底池可配置化**：从兜底池卡片一键选择"为哪些池兜底"，auto 池强制兜底（服务端兜底）
- **多模态感知**：请求含图片时自动跳过纯文本模型，选用多模态模型；无可用多模态时明确拒绝
- **双计费模式**：按 Token 或按请求次数计费；人工标注免费（绿）/付费（红）
- **一次性令牌**：到期或用完即永久失效，不刷新
- **API 密钥自动轮换**：用户密钥可设过期时长，到期自动生成新 secret，旧 secret 宽限期内仍可认证
- **双管理后台**：传统 `/admin` 与科技感 `/hfadmin`，共用同一套后端 API
- **拖拽排序**：池内模型、模型管理列表、池/供应商顺序均可拖拽并持久化
- **流式支持**：SSE 流式响应透传，Anthropic 自动转换为 OpenAI chunk 格式；同时支持客户端以 Anthropic Messages 格式调用（自动转换，纯文本块列表会被合并为字符串以适配上游）
...
├── static/
│   ├── index.html            # 管理后台单页应用
│   └── hfadmin.html          # 科技感管理面板（与 /admin 共用后端 API）
├── requirements.txt
└── gateway.db                # 运行时自动生成
```
...
- **自动择优模式**：在可用模型中按**实时延迟从低到高**优先（延迟来自测速 + 真实请求耗时的滑动平均）；超过池配置 `slow_latency_threshold`（默认 3000ms，0=不限）的模型直接排到末尾
...
**兜底池回退**：当主池（及其子池）所有模型都不可用时，自动回退到该池 `fallback_pool` 指定的兜底池。可在管理后台从兜底池卡片一键勾选"为哪些池兜底"（auto 池强制兜底）；未勾选的池其 `fallback_pool` 自动清空。
...
- **`/admin/`** — 传统表格风格管理后台
- **`/hfadmin`** — 科技感 / 卡片风格面板（v2.1bate 引入，新功能优先在此落地）

任选其一访问即可；模型/池的 CRUD、拖拽排序、用量统计、接口测速、调用记录全部可用。
...
| **用量统计** | 卡片式可视化，横向进度条展示 已用/总量（青→琥珀→红分级），支持计费量 / 调用次数 / Token 用量三维度切换，每 5 秒自动刷新 |
| **接口测速** | 并发测试所有模型的延迟与吞吐 |
...
- 部分上游服务对纯文本块列表 `content: [{type:text, text:"..."}]` 兼容性差，本网关已自动合并为字符串（v2.3bate）
